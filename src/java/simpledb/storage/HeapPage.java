...
public class HeapPage implements Page {
    ...
    */
    private int getNumTuples() {        
        ...
        // tuple size * 8 for content; 1 bit for header
...
//        return (int) Math.ceil(this.numSlots / 8);
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        if (!t.getRecordId().getPageId().equals(pid)) {
            throw new DbException("The tuple is not on this page " + pid.getPageNumber());
        }
        // update header
...
//        return numSlots - cnt;
    ...
    public boolean isSlotUsed(int i) {
        // some code goes here
//        int idx = i / 8;
//        String s = Integer.toBinaryString(header[idx]);
//        String s1 = Integer.toBinaryString(1 << (header[idx] - 1));
//        if ((header[idx] & (header[idx] - 1)) == header[idx] << (i % 8)) {
//            return true;
//        }
//        return false;
    ...
    private void markSlotUsed(int i, boolean value) {
        // some code goes here
        if (value) {
            header[i>>3] = (byte) (header[i>>3] | (1 << (i & 7)));
        } else {
            // 1<<(i&7) ==> 0...01..00
            // -1       ==> 1...11..11
            // ^        ==> 1...10..11
            header[i>>3] = (byte) (header[i>>3] & (-1 ^ (1 << (i & 7)) ));
        }
    }
