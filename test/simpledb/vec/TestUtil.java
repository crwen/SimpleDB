...
public class TestUtil {
     ...
     */
    public static TupleIteratorVec createTupleList(int width, int[] tupdata) {
        int i = 0;
        ...
        TupleIteratorVec result = new TupleIteratorVec(Utility.getTupleDesc(width), tuplist);
        result.open();
        return result;
    }
     ...
     */
    public static void compareDbIterators(OpIteratorVec expected, OpIteratorVec actual)
            throws DbException, TransactionAbortedException {
        while (expected.hasNext()) {
            assertTrue(actual.hasNext());
            Chunk expectedChunk = expected.next();
            Chunk actualChunk = actual.next();
//            Tuple expectedTup = expected.next();
//            Tuple actualTup = actual.next();

            for (int i = 0; i < actualChunk.getTuples().size(); i++) {
                Tuple actualTup = actualChunk.getTuples().get(i);
                Tuple expectedTup = expectedChunk.getTuples().get(i);
...
//            assertTrue(compareTuples(expectedTup, actualTup));
     ...
     * If not, throw an assertion.
     */
    public static void matchAllTuples(OpIterator expected, OpIteratorVec actual) throws
        ...
        while (expected.hasNext()) {
            ...
            while (actual.hasNext()) {
                for (Tuple next : actual.next().getTuples()) {
...
//                }
     ...
     */
    public static boolean checkExhausted(OpIteratorVec it)
...
//            Tuple t = it.next();
     ...
     */
    public static class MockScan implements OpIteratorVec {
        private int cur;
		...
		public Chunk next() throws NoSuchElementException {
			if(cur >= high) throw new NoSuchElementException();
            Chunk chunk = new Chunk(getTupleDesc());
